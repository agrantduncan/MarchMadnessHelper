...
import { SEED_WIN_RATES, ROUND_LABELS, ROUND_NAMES } from '../data/seedWinRates'
...
export default function StatsPanel({ teamId, onClose }) {
      ...
      {team && (
        ...
        >
          ...
          <div className="p-4 border-b border-slate-700 flex items-start justify-between">
            ...
            <button
              onClick={onClose}
              className="text-slate-500 hover:text-slate-300 transition-colors ml-2 mt-0.5 text-lg leading-none"
            >
              ✕
            </button>
          </div>
                      ...
                      <div className="flex-1 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                        <motion.div
                          className={`h-full rounded-full ${
                            barWidth >= 60 ? 'bg-green-500' : barWidth >= 30 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}
                          initial={{ width: 0 }}
                          animate={{ width: `${barWidth}%` }}
                          transition={{ duration: 0.4, delay: 0.05 * ROUND_LABELS.indexOf(label) }}
                        />
                      </div>
